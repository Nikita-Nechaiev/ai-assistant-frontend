export const getTimeDiffString = (diffInMs: number): string => {
  ...
  if (totalHours < 24) {
    ...
    if (remainingMinutes > 0) {
      result += ` and ${remainingMinutes} minute${
...
export const getExpirationMessage = (expiresAt?: string | null): string => {
  ...
  return expirationDiff < 0
    ? `The invitation expired ${expiresStr} ago.`
    : `The invitation expires in ${expiresStr}.`;
};
